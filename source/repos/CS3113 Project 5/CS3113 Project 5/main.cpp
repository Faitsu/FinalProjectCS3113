...
void ProcessInput() {
	...
	while (SDL_PollEvent(&event)) {
		...
		case SDL_KEYDOWN:
			...
			case SDLK_UP:
				if (currentScene->state.player-> jumpcount < currentScene->state.player->maxjumpcount) {
					currentScene->state.player->fly = true;
					currentScene->state.player->jumpcount++;
					currentScene->state.player->airborne = true;
	...
	if (keys[SDL_SCANCODE_LEFT]) {
		...
		if (!currentScene->state.player->airborne) {
